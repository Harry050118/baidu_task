...
- 新增 4 个只读接口：`/api/stats/overview`、`/api/stats/stations`、`/api/status/data`、`/api/imports/latest`。
...
### 2.2 `GET /api/stats/stations`

用途：为数据状态页或统计面板提供真实站类分布。
...
该接口只做站类分布统计，不做行政区统计。当前缺少经纬度和行政区归属，行政区统计延后到坐标补全或审核之后。

### 2.3 `GET /api/status/data`
...
- `/api/stats/stations` 返回真实站类分布，并明确行政区统计当前不可用。
