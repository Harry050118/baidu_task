...
> 当前优先级：先完成积水监测主线闭环。告警中心、用户角色审计、处置工单、预测展示等能力保留设计位置，但不作为当前阶段必须先完成的内容。
...
```text
数据采集与归档 -> SQLite 入库 -> 坐标候选与审核 -> 风险趋势计算 -> FastAPI 主线接口 -> Vue + 高德地图展示 -> 统计/历史/校核/数据状态页面
```
...
这些分支能力只保留数据模型、接口和页面设计位置，后续在主线稳定后再实现。
...
后续增强可以在 PostGIS 中补齐降雨、道路、历史易涝点、告警、工单、用户审计和预测结果。以下表当前不要求先落地，只保留顶层设计，避免后续接口和页面反复返工。
...
以下接口为后置增强接口，继续放在 `/api/` 下，保持前后端分离。当前主线先实现地图、统计、历史、坐标校核和数据状态接口；调度、阈值、告警、工单、预测、用户审计等接口不先实现。
...
后置后台页面围绕防汛处置增强能力展开，不追求复杂管理平台。当前主线优先完成历史查询、点位校核和数据状态页面；告警、阈值、调度、工单和审计页面只保留设计。
...
预测展示属于后置分支。本轮不要求实现预测页面或预测接口；后续若展示规则型预测结果，必须带 `model_version` 和生成时间，避免被误解为实时权威预报。
...
- 告警、工单、预测未实现：不影响主线地图、统计、历史、校核和数据状态交付。
...
其中告警、用户角色审计、工单和预测展示不作为当前主线完成条件。
...
- 后置调度、阈值、告警、图层、工单、预测、审计 API 已明确，但不作为当前主线实现要求。
- 指挥地图、历史查询、点位校核、数据状态页面已明确。
- 告警中心、阈值配置、调度状态、审计日志、工单、预测和扩展图层页面已明确为后置页面。
- 数据同步主链路已明确，告警处置、扩展图层和预测展示为后置链路。
